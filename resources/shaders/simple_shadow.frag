...
{ 
  ...
  const float lightAngleCos = dot(normalize(Params.spotlightDirection), -lightDir);
  const float intensityFraction = smoothstep(Params.spotlightCutoffCos, Params.spotlightInnerCos, lightAngleCos);
  ...
  out_fragColor = intensityFraction * (lightColor*shadow + vec4(0.1f)) * vec4(Params.baseColor, 1.0f);
}
