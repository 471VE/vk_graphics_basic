...
void main()
{
  ...
  if (Params.vsmEnabled) {
    vec2 expectations = textureLod(varianceShadowMap, shadowTexCoord, 0).xy;
    float mean = expectations.x;
    float deviation = posLightSpaceNDC.z - mean;
    float variance = expectations.y - mean*mean;
    float p_max = variance/(variance + deviation*deviation);
    shadow  = ((posLightSpaceNDC.z < mean + 0.001f) || outOfView) ? 1.0f : p_max;
  } else
